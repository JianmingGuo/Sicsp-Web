...
def contactme(req):
    name = req.POST.get('name2', None)
    email = req.POST.get('email2', None)
    message = req.POST.get('message2', None)
    if email:
        msg = name + '\n' + email + '\n' + message + '\n'
        send_mail('From Sicsp-Identity', msg, settings.EMAIL_HOST_USER,
